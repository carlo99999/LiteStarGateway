...
> requested `max_tokens` output ceiling, released at settlement), so a burst of
> concurrent requests — streams especially — can't all pass under a
> nearly-exhausted limit. The residual overshoot is: requests already in flight
