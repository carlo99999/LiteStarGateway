...
> both wired into `CompletionService`. Provider-aware structured-output
> translation and fail-loud capability gating are also in
> (`infrastructure/llm/structured_output.py`, see §3.1).
...
- **Allowlist per operation** (deny by default). Indicative sets:
  ...
  - responses: the request-body fields exposed by the installed OpenAI SDK,
    ...
    represent. All accept text and sampling. Databricks, Anthropic and Vertex
    accept schema-constrained output; Bedrock accepts best-effort `json_object`
    and rejects `json_schema` until its per-model capability matrix is explicit.
    Databricks, Anthropic and capability-gated Bedrock Claude 3/Nova models accept
    non-streaming function tools and stateless function-call/result replay.
...
- **Bedrock**: `json_object` is a best-effort system instruction; `json_schema`
  is rejected until the native `outputConfig.textFormat` model matrix is
  explicit. A non-strict forced tool is not treated as schema enforcement.
