...
def usd(amount: str) -> Decimal:
    """Money as the operator would write it — the tests read as amounts, not as
    conversions."""
    return to_cost(amount)
...
class TestCrossedThresholds:
    ...
    def test_zero_or_negative_cap_never_fires(self) -> None:
        ...
        assert (
            crossed_thresholds(
                spend=usd("10"), limit_cost=usd("0"), thresholds=[50, 80], fired=set()
            )
            == []
        )
    ...
    def test_empty_threshold_list_never_fires(self) -> None:
        assert (
            crossed_thresholds(spend=usd("100"), limit_cost=usd("100"), thresholds=[], fired=set())
            == []
        )
