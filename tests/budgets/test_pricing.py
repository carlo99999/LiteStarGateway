...
@pytest.mark.parametrize(
    ...
    [
        (0, 0, 0.001, 0.002, Decimal("0")),
        (1000, 0, 0.001, 0.002, Decimal("1")),
        (0, 500, 0.001, 0.002, Decimal("1")),
        (1000, 500, 0.001, 0.002, Decimal("2")),
        (100, 200, 0.00003, 0.00006, Decimal("0.015")),  # 100*3e-5 + 200*6e-5
    ],
)
def test_token_only_cost_matches_prompt_input_plus_completion_output(
    prompt: int, completion: int, input_rate: float, output_rate: float, expected: Decimal
) -> None:
    usage = BillableUsage(prompt_tokens=prompt, completion_tokens=completion)
    rates = RateCard(
        input_cost_per_token=money(input_rate), output_cost_per_token=money(output_rate)
    )
    assert compute_cost(usage, rates) == expected


def test_unpriced_model_bills_zero_for_tokens() -> None:
    usage = BillableUsage(prompt_tokens=1000, completion_tokens=1000)
    assert compute_cost(usage, RateCard()) == ZERO_MONEY
...
def test_cache_tokens_priced_at_their_own_rates() -> None:
    ...
    rates = RateCard(
        ...
        cache_write_cost_per_token=money(0.00125),  # ~1.25x input (Anthropic economics)
        cache_read_cost_per_token=money(0.0001),  # ~0.1x input
    )
    expected = (
...
def test_cache_tokens_unbilled_when_no_cache_rates_configured() -> None:
    ...
    rates = RateCard(input_cost_per_token=money(0.001))
    assert compute_cost(usage, rates) == Decimal("0.1")
...
def test_three_images_at_flat_per_image_rate() -> None:
    ...
    rates = RateCard(image_cost_per_image=money(0.04))
    assert compute_cost(usage, rates) == Decimal("0.12")
...
def test_size_quality_specific_price_overrides_flat_fallback() -> None:
    ...
    rates = RateCard(
        image_cost_per_image=money(0.04),
        image_prices={"1024x1024/hd": money(0.08), "1024x1024/standard": money(0.04)},
    )
    assert compute_cost(usage, rates) == Decimal("0.16")  # 2 * 0.08
...
def test_unmatched_size_quality_falls_back_to_flat_rate() -> None:
    ...
    rates = RateCard(image_cost_per_image=money(0.02), image_prices={"1024x1024/hd": money(0.08)})
    assert compute_cost(usage, rates) == Decimal("0.02")
...
def test_missing_dimension_uses_flat_fallback() -> None:
    ...
    rates = RateCard(
        image_cost_per_image=money(0.03),
        image_prices={"1024x1024/standard": money(0.05)},
    )
    assert compute_cost(usage, rates) == Decimal("0.06")


def test_image_unit_price_helper() -> None:
    rates = RateCard(
        image_cost_per_image=money(0.01),
        image_prices={"1024x1024/hd": money(0.09)},
    )
    assert image_unit_price(rates, "1024x1024", "hd") == Decimal("0.09")
    assert image_unit_price(rates, "512x512", "standard") == Decimal("0.01")
    assert image_unit_price(RateCard(), "512x512", "standard") == ZERO_MONEY
...
def test_all_dimensions_sum_independently() -> None:
    ...
    rates = RateCard(
        input_cost_per_token=money(1.0),
        output_cost_per_token=money(2.0),
        cache_write_cost_per_token=money(3.0),
        cache_read_cost_per_token=money(4.0),
        image_cost_per_image=money(5.0),
    )
    expected = Decimal(10 * 1 + 20 * 2 + 30 * 3 + 40 * 4 + 2 * 5)
    assert compute_cost(usage, rates) == expected
