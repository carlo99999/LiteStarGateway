...
async def test_a_vector_at_the_threshold_hits() -> None:
    ...
    await cache.add(TEAM, KEY, [1.0, 0.0], _value("a"), ttl_s=60)

    found = await cache.find(TEAM, KEY, [1.0, 0.0], threshold=0.97)
...
async def test_a_vector_below_threshold_misses() -> None:
    ...
    found = await cache.find(TEAM, KEY, [0.0, 1.0], threshold=0.97)
...
async def test_a_different_team_never_matches_even_with_an_identical_vector() -> None:
    ...
    found = await cache.find(OTHER_TEAM, KEY, [1.0, 0.0], threshold=0.97)
...
async def test_a_different_api_key_never_matches_within_the_same_team() -> None:
    ...
    found = await cache.find(TEAM, OTHER_KEY, [1.0, 0.0], threshold=0.97)
...
async def test_an_expired_entry_is_a_miss() -> None:
    ...
    assert await cache.find(TEAM, KEY, [1.0, 0.0], threshold=0.97) is None


async def test_bucket_is_bounded_per_tenant() -> None:
    cache = InMemorySemanticResponseCache(max_entries_per_tenant=2)
    await cache.add(TEAM, KEY, [1.0, 0.0, 0.0], _value("a"), ttl_s=60)
    await cache.add(TEAM, KEY, [0.0, 1.0, 0.0], _value("b"), ttl_s=60)
    await cache.add(TEAM, KEY, [0.0, 0.0, 1.0], _value("c"), ttl_s=60)

    # The oldest ("a") was evicted; only "b" and "c" remain findable.
    assert await cache.find(TEAM, KEY, [1.0, 0.0, 0.0], threshold=0.5) is None
    assert await cache.find(TEAM, KEY, [0.0, 0.0, 1.0], threshold=0.99) == _value("c")


async def test_the_closest_entry_above_threshold_wins() -> None:
    cache = InMemorySemanticResponseCache()
    await cache.add(TEAM, KEY, [1.0, 0.1], _value("closer"), ttl_s=60)
    await cache.add(TEAM, KEY, [1.0, 0.5], _value("farther"), ttl_s=60)

    found = await cache.find(TEAM, KEY, [1.0, 0.0], threshold=0.5)
