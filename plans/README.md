...
| 11 | [Platform quality gates](11-platform-quality-gates.md) | 🚧 Slice D (dependency ceiling) and Slice A (request correlation) complete — `mlflow-skinny` pinned `>=3.14,<4` plus a scheduled non-blocking next-major compatibility workflow; `X-Request-ID` middleware bound to structlog and propagated to trace/usage/audit/routing-decision records via `request_context.current_request_id()`, trusted-proxy-gated inbound acceptance, migration for the four new `request_id` columns; Slices B/C (drift gates, browser E2E) not yet landed on `main` | Request correlation, drift gates, browser E2E, dependency safety |
