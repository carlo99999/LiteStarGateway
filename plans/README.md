...
| 04 | [Response caching](04-response-caching.md) | 🚧 Phase 0 + 1 + 2 complete (exact-match, Redis-backed shared store selected by `REDIS_URL` + in-memory fallback, synthetic-stream replay for cached chat completions, semantic tier at cosine ≥ `RESPONSE_CACHE_SEMANTIC_THRESHOLD` tried only on an exact-match miss, off by default + per-team/model opt-in for both tiers, `cache_hit` metered at $0); Phase 3 (console) remains | Cost & latency — exact-match + optional semantic cache, per-tenant isolated |
