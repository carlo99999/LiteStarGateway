...
| 09 | [Responses API Level B](09-responses-level-b.md) | 🚧 Phase 0 + 1a + Anthropic 1b-A complete; Bedrock next | Contract correctness — faithful tool events on chat-only providers, fail loudly otherwise |
...
1. **Correctness now:** Plan 09 Phase 1b-B (extend the faithful non-streaming
   tool contract from Databricks/Anthropic to capability-gated Bedrock), then
   resolve Vertex thought-signature replay and implement the streaming event
   contract; Plan 10 Phase 0 (attach stream usage to routing decisions).
