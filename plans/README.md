...
| 04 | [Response caching](04-response-caching.md) | 🚧 Phase 0 + 1 complete (exact-match, Redis-backed shared store selected by `REDIS_URL` + in-memory fallback, synthetic-stream replay for cached chat completions, off by default + per-team/model opt-in, `cache_hit` metered at $0); Phases 2–3 (semantic tier, console) remain | Cost & latency — exact-match + optional semantic cache, per-tenant isolated |
