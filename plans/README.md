...
| 11 | [Platform quality gates](11-platform-quality-gates.md) | 🚧 Slices A, B and D complete — `X-Request-ID` middleware bound to structlog and propagated to trace/usage/audit/routing-decision records via `request_context.current_request_id()`, trusted-proxy-gated inbound acceptance, migration for the four new `request_id` columns; `mlflow-skinny` pinned `>=3.14,<4` plus a scheduled non-blocking next-major compatibility workflow; CI now gates on `just migration-check`, an OpenAPI/TypeScript schema-drift diff, and an internal Markdown link checker for `plans/`/`docs/`. Slice C (browser E2E) not yet landed on `main` | Request correlation, drift gates, browser E2E, dependency safety |
...
| 13 | [Billing integrity & retention](13-billing-integrity.md) | 🚧 Phase 1 shipped (non-token pricing) | Image/cache-token pricing, decimal money, distributed budgets, durable history |
