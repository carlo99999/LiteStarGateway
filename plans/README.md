...
| 05 | [Cross-provider failover](05-cross-provider-failover.md) | 🚧 Phase 0 + 1 + 2 + Phase 3 observability complete (non-streamed and streaming failover, pre-first-byte only, config, RPM/budget single-charge guarantees, persisted attempts/failover_used); optional circuit breaker + console view not started | Reliability — fall over to another capable candidate on 429/5xx/timeout |
