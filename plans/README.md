...
| 05 | [Cross-provider failover](05-cross-provider-failover.md) | 🚧 Phase 0 + 1 + 2 complete (non-streamed and streaming failover, pre-first-byte only, config, RPM/budget single-charge guarantees); Phase 3 (circuit breaker + observability) next | Reliability — fall over to another capable candidate on 429/5xx/timeout |
