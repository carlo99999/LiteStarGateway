...
| 07 | [Budget alerts](07-budget-alerts.md) | 🚧 Phase 0 + 1 + 2 complete (threshold model + dedup ledger + settlement evaluation + delivery outbox + webhook channel + background dispatch worker); Phase 3 remaining (email channel + console config) | Proactive spend notifications at % thresholds, off the hot path |
