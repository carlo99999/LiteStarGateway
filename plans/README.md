...
| 05 | [Cross-provider failover](05-cross-provider-failover.md) | ✅ Complete — non-streamed and streaming failover (pre-first-byte only), config, RPM/budget single-charge guarantees, persisted attempts/failover_used, in-memory/Redis breaker skipping tripped candidates, and the reliability view: `GET /teams/{id}/routers/{rid}/reliability` (attempts histogram, failover rate, live breaker state per candidate via a read-only `CircuitBreakerInspector` that never claims the half-open trial) rendered on the console's router detail page | Reliability — fall over to another capable candidate on 429/5xx/timeout |
| 06 | [Guardrails](06-guardrails.md) | ✅ Complete — ordered per-team chains inspecting a prompt before dispatch (before any budget is reserved) and an answer after settlement (so a blocked answer is still billed); `webhook` (HMAC-signed, SSRF-guarded, strictly parsed) and `judge` (gateway-hosted model, billed as `guardrail.judge`) providers; per-provider fail-open/closed; redaction applied only where the mapping is exact, escalated to a block otherwise; team/model-scoped policy CRUD with write-only signing secrets under `guardrails:read`/`guardrails:manage`; `GuardrailBlocked` → 422 plus an `error` trace; console chain editor at `/guardrails`. See [docs/guardrails.md](../docs/guardrails.md). Open: no `guardrail.block` feed (traces are not queryable) and no response-side guarding of streams | Enterprise policy — pluggable PII/moderation pre- and post-call |
...
| 13 | [Billing integrity & retention](13-billing-integrity.md) | 🚧 Phases 1, 3, 4, 5 complete — image/cache-token pricing via one normalized pricing function; fleet-wide budget reservations (Redis Lua CAS, scope-keyed) so a concurrent burst cannot slip under a nearly-exhausted cap; per-API-key caps with `block`/`alert`; retention lifecycle (soft-delete, export, audited purge). Phase 2 (decimal money) is **deliberately partial**: rates and costs are `Decimal` end to end, the reservation store's bounded comparison stays float — see [Plan 17](17-post-round-13-execution.md), Slice 5 | Image/cache-token pricing, decimal money, distributed budgets, durable history |
...
| 19 | [Round 15 remediation](19-round-15-remediation.md) | ✅ complete (#442–#455) — all nineteen findings closed, one PR per slice; eight slices deviated from the plan and the deviations are recorded in its **State at close** section | One PR per finding, each with a regression that fails first |
| 20 | [MCP tool gateway](20-mcp-tool-gateway.md) | ⏳ designed, not started | Governed tool execution — MCP servers as team resources with three visibilities, bounded rounds inside the egress/audit/spend perimeter, and guardrails on tool calls and results |
