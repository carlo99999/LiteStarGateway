...
| 09 | [Responses API Level B](09-responses-level-b.md) | 🚧 Phase 0 + 1a complete; provider-native chat tools next | Contract correctness — faithful tool events on chat-only providers, fail loudly otherwise |
...
1. **Correctness now:** Plan 09 Phase 1b (extend the faithful non-streaming tool
   contract from Databricks to Anthropic, Vertex and Bedrock), then its streaming
   event contract; Plan 10 Phase 0 (attach stream usage to routing decisions).
