...
| 13 | [Billing integrity & retention](13-billing-integrity.md) | 🚧 Phases 1, 3, 4, 5 complete — image/cache-token pricing via one normalized pricing function; fleet-wide budget reservations (Redis Lua CAS, scope-keyed) so a concurrent burst cannot slip under a nearly-exhausted cap; per-API-key caps with `block`/`alert`; retention lifecycle (soft-delete, export, audited purge). Phase 2 (decimal money) is **deliberately partial**: rates and costs are `Decimal` end to end, the reservation store's bounded comparison stays float — see [Plan 17 Slice 5](17-post-round-13-execution.md#slice-5--decimal-money-2-prs-closed-early--see-below) | Image/cache-token pricing, decimal money, distributed budgets, durable history |
