"""add sso_subject to user_account

Revision ID: c1a2b3d4e5f6
Revises: b84683c8f499
...
def schema_upgrades() -> None:
    ...
    with op.batch_alter_table('user_account', schema=None) as batch_op:
        batch_op.add_column(sa.Column('sso_subject', sa.String(), nullable=True))
        batch_op.create_index(batch_op.f('ix_user_account_sso_subject'), ['sso_subject'], unique=True)

def schema_downgrades() -> None:
    """schema downgrade migrations go here."""
    with op.batch_alter_table('user_account', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_user_account_sso_subject'))
